...
<template>
  <div id="answer">
    <div>
      <video id="video" width="300px" preload autoplay loop muted style="display: none;"></video>
      <canvas id="canvas" width="250" height="250" style="position: absolute; top: 0; left: -999999"></canvas>
    </div>
    ...
    facelogin() {
      ...
      this.$axios
        ...
        .catch(function (error) {
          _this.$message.error("请未规范答题要求，未识别到人脸，一分钟后将自动提交试卷，已0分处理");
          if (_this.txtime == 2) {
            let date = new Date()
            _this.endTime = _this.getTime(date)
            let answerDate = _this.endTime.substr(0, 10)
            //提交成绩信息
            _this.$axios({
              url: '/api/score',
              method: 'post',
              data: {
                examCode: _this.examData.examCode, //考试编号
                studentId: _this.userInfo.id, //学号
                subject: _this.examData.source, //课程名称
                etScore: 0, //答题成绩
